...
На небольших экранах учитывать `safeAreaInsetBottom`. Переходы должны сохранять выбранную
дату и контекст списков там, где это ожидаемо. Повторения открываются как вложенный экран
профиля и не добавляют шестую основную вкладку.
...
Выбранные вкладка, тип, характеристика и поисковый запрос хранятся в URL и переносятся в
детали для точного возврата к списку. Активные квесты с дедлайном сортируются от
...
Поисковый запрос и выбранная характеристика хранятся в URL и переносятся в детали навыка,
чтобы возврат не сбрасывал состояние списка.
...
- настройки темы;
- часовой пояс;
- переход на отдельный экран управления повторениями;
- экспорт данных в JSON;
- выход из серверной сессии.

Вложенный экран «Повторения» содержит активные и приостановленные шаблоны, редактирование,
паузу, архив и восстановление. Поиск и фильтр по навыку хранятся в URL. Нижняя навигация на
этом экране остаётся пятираздельной, активным считается профиль.
